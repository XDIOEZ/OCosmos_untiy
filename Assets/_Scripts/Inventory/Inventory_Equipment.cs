...
public class Inventory_Equipment : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        inventory.Data.inventoryName = "装备插槽";
    ...
    public void EquipArmor(int slotIndex, ItemSlot InputSlot)
    {
        ...
        Debug.Log("装备的不是盔甲。而是:" + InputSlot._ItemData.ItemTags.Type_Base);
        Debug.Log(slotIndex + " 号槽位的装备类型与槽位类型不匹配。");
        Debug.Log("装备的防具类型与槽位防具类型不匹配。" + InputSlot._ItemData.ItemTags.Type_Sub + " 与 " + inventory.Data.itemSlots[slotIndex].CanAcceptItemType.Type_Sub);
    } 
}
